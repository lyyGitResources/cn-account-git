...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		<title>
			<s:i18n name="com.hisupplier.cn.account.product.package"><s:text name="trade.listTitle"></s:text></s:i18n>
			<s:if test="result.group.groupId > 0">
				（<s:property value="result.group.groupName"/>）
			</s:if>
			<s:else>
				（<s:text name="group.noGroupTradeTitle"/>）
			</s:else>
		</title>
		...
		<script type="text/javascript">
			...
			function selectGroup(groupId,groupName,groupNamePath) {
				if (!Util.isChecked("proId")) {
					window.alert('<s:text name="group.selectTrade"/>');
					
				} else if(groupId == <s:property value="result.group.groupId"/>){
					alert("商情已属于该分组，请重新选择分组");
					
				}else if(groupId == 0){
					document.productForm.action = "/group/remove_from_trade_group.htm";
					...
					<s:if test="result.group.groupId > 0">
						document.productForm.action = '/group/change_trade_group.htm';
						document.productForm.groupId.value = <s:property value="result.group.groupId"/>;
						document.productForm.newGroupId.value = groupId;
					</s:if>
					<s:else>
						document.productForm.action = '/group/select_trade_group.htm';
			...
			function deleteTrade(){
				if(!Util.isChecked("proId")){
					alert('<s:text name="group.selectTrade"/>');
				}else{
					if(confirm("<s:text name='deleteConfirm' />")){
						...
						document.productForm.groupId.value = <s:property value="result.group.groupId"/>;
						document.productForm.action = '/group/trade_delete.do';
						document.productForm.submit();
					}
				}
			}
	...
	<body>
		...
		<form class="searchForm" id="searchForm">
			<table>
				<tr>
					<td>
						<input name="queryText" id="queryText" value="<s:i18n name="com.hisupplier.cn.account.product.package"><s:text name='trade.queryText'/></s:i18n>" style="width:200px;" oriValue="<s:i18n name="com.hisupplier.cn.account.product.package"><s:text name='trade.queryText'/></s:i18n>"/>
					</td>
		...
		<div class="buttonLeft">
			<input type="button" onclick="javascript:deleteTrade();" value='<s:text name="button.removeTradeFromGroup"/>' />
			<s:if test="result.group.groupId > 0">
				<input type="button" onclick="javascript:showGroupList();" value='<s:text name="button.removeToOtherTraGroup"/>' />
			</s:if>
			<s:else>
				<input type="button" onclick="javascript:showGroupList();" value='<s:text name="button.removeToOtherTraGroup"/>' />
			</s:else>
